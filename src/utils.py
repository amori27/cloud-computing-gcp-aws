...
def format_cloudwatch_log(stream_name: str, log_data: str) -> dict[str, Any]:
    ...
    Returns:
        Formatted log entry.
    """
    return {"logStreamName": stream_name, "logData": log_data, "timestamp": None}
...
def generate_vm_config(
    name: str, machine_type: str, zone: str, disk_size: int = 100
) -> dict[str, Any]:
    ...
    return {
        ...
        "zone": zone,
        "disk": {"size_gb": disk_size, "type": "pd-standard"},
        "network": {"interface": "NIC0", "network": "default"},
    }


def calculate_cloud_cost(
    instance_count: int, machine_type: str, hours_per_month: float = 730
) -> dict[str, float]:
    ...
    Returns:
        Cost breakdown.
    """
    base_costs = {"small": 0.05, "medium": 0.10, "large": 0.20}
